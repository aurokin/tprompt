...
func TestResolveConflictsGlobalOnly(t *testing.T) {
	...
	got, err := resolveConflicts(sources, ConflictPolicyGlobalOnly)
	if err != nil {
		t.Fatalf("resolveConflicts: %v", err)
	}
	gotWinners := winnerPathsByID(got.Winners)
	wantWinners := map[string]string{
		"alpha": "/team/alpha.md",
		"bravo": "/primary/bravo.md",
	}
	if diff := cmp.Diff(wantWinners, gotWinners); diff != "" {
		t.Fatalf("winners mismatch (-want +got):\n%s", diff)
	}
	if len(got.Shadows) != 0 {
		t.Fatalf("Shadows = %#v, want empty for global-only slice", got.Shadows)
	}
}

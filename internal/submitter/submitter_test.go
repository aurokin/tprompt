...
func TestSubmit_ActionClipboardNotYetImplemented(t *testing.T) {
	fs := &fakeStore{}
	dc := &fakeDaemonClient{}

	s := New(fs, dc, baseCfg(), baseTarget())
	err := s.Submit(tui.Result{Action: tui.ActionClipboard, ClipboardBody: []byte("clip")})
	if err == nil {
		t.Fatal("want stub error for AUR-22 clipboard path, got nil")
	}

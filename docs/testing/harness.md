...
The import subsystem is generic over an `ImportRecord` and registers sources
through an `ImportSource` seam (interfaces.md). The seam is proved without the
...
- the source seam is generic: a non-wispr `fakeImportSource`/`fakeRecord` imports

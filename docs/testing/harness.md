...
(DECISIONS §34) is tested through the real cobra command tree: cobra owns root
persistent flags, unknown source flags, explicit source names, stray positionals,
and malformed flags, while the import parent dispatches only after a truly bare
invocation reaches its `RunE`.
...
- bare-import dispatch: inside tmux + interactive tty, bare `import` dispatches to
  ...
  **not** dispatch a named source (`import wispr`), a source flag (`import
  ...
  value-taking root flag (`import --config`), or a non-interactive stream
  (redirected stdout / injected non-tty); outside tmux or without a tty it is left
  to print help

...
app-level tests with a fake `Reader` (`internal/app/import_test.go`), exit-code
mapping (`internal/app/exit_test.go`), and `testscript` against a committed
fixture `flow.sqlite` (`cmd/tprompt/testdata/script/import_wispr_*.txtar`). The

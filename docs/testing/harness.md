...
plan/classification tests (`internal/app/import_plan_test.go`), exit-code mapping
...
- the write-free classifier (`classifySnippet`/`dryRunPlan`, the seam the import TUI
  will render) matches the scriptable writer: the three §34 cases classify correctly,

...
| Body and frontmatter parsing | `internal/promptmeta/promptmeta_test.go`: frontmatter/body split, no-frontmatter body, CRLF, UTF-8 BOM, unknown fields, malformed mapping rejection, non-frontmatter fences, single leading and single trailing newline trim, and template variable parsing. Store tests verify delivered body, prompt defaults, and template validation. | Covered |
| Template variables | `internal/prompttmpl/prompttmpl_test.go`: variable-name validation, reserved-name rejection, unused-variable rejection, placeholder validation, escaping, defaults, required values, and rendering. `internal/app/send_test.go` covers prompt-specific CLI flags; `internal/tui/model_test.go` covers one-at-a-time interactive input; `cmd/tprompt/testdata/script/send_template_*.txtar` covers black-box pre-delivery failures. | Covered |
...
| Known limitations documentation | `README.md`, `EXPECTATIONS.md`, `docs/roadmap/future-phases.md`, and command docs describe same-host clipboard, single-character keybinds, no live clipboard preview, no remote delivery, and no prompt composition/snippets. AUR-113 owns the final docs refresh after Phase 6/7 behavior lands. | Covered for current MVP; final refresh deferred to AUR-113 |
